...
class FixedMultiClientDownloadAnalyzer:
    def __init__(self, root):
        self.root = root
        self.root.title("Fixed Multi-Client Download Analyzer")
        self.root.geometry("1400x900")
        
        # Client configuration
        self.client_id = str(uuid.uuid4())[:8]
        self.client_name = f"Client_{socket.gethostname()}_{self.client_id}"
        self.server_url = "http://172.20.225.22:5000"
                        ...
                        try:
                            ...
                            if response.status_code == 200:
                                ...
                                try:
                                    ...
                                    self.log_message("Invalid JSON from server")
                            else:
                                self.log_message(f"Server error: {response.status_code}")
                        except requests.RequestException as e:
                            self.log_message(f"Request error: {str(e)}")
        ...
        try:
            ...
            if 'clients' in data and data['clients']:
                for client in data['clients']:
                    status = "Online" if client.get('active', False) else "Offline"
                    avg_speed = client.get('avg_speed', 0)
                    test_count = client.get('test_count', 0)
                ...
                self.log_message(f"Updated with {len(data['clients'])} clients")
            else:
                self.log_message("No client data received")
                
        except Exception as e:
            self.log_message(f"Error updating view: {str(e)}")
    ...
    def update_multi_plots(self, data):
        try:
            ...
            active_clients = [c for c in clients if c.get('data') and c.get('avg_speed', 0) > 0]
            ...
            for i, client in enumerate(active_clients):
                speeds = [d['speed'] for d in client.get('data', [])]
                if speeds:
            ...
            self.log_message(f"Error updating plots: {str(e)}")
...
if __name__ == "__main__":
    root = tk.Tk()
    app = FixedMultiClientDownloadAnalyzer(root)
    root.mainloop()
